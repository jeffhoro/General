...
# Sys.setenv(RETICULATE_PYTHON = "~/.virtualenvs/r-reticulate/Scripts/python.exe")
...
# Pre-Processing
# Convert Week and Year into Date Format
data_clean <- data %>%
  mutate(
    week_start_date = as.Date(paste(year, week, 1, sep = "-"), format = "%Y-%U-%u")
  ) %>%
  select(week_start_date, everything()) # Move date column to the front

# Drop year and week since we already created the week date
data_clean <- data_clean %>%
  select(-year, -week)

data_clean <- data_clean %>%
  select(-clicks, -value_from_conversions)

data_clean <- data_clean %>%
  select( -publisher_group)

# Generate Paid Media Channels
data_formatted <- data_clean %>%
  mutate(
    paid_media_channels = paste(
      #tolower(gsub(" ", "", publisher_group)),   
      tolower(gsub(" ", "", device)),      
      tolower(gsub(" ", "", supply_type)),
      #tolower(gsub(" ", "", platform)),
      #tolower(gsub(" ", "", browser)),
      #tolower(gsub(" ", "", operating_system)),
      sep = "_"
    )
  ) %>%
  select(-device, -supply_type) %>% # Drop original columns # Dropped , -browser, -operating_system
  select(week_start_date, paid_media_channels, everything()) # Move paid_media_channels forward

# Get the distinct values in the "paid_media_channels" column
distinct_channels <- unique(data_formatted$paid_media_channels)
# Count the number of distinct values
number_of_distinct_channels <- length(distinct_channels)
# Print the result
print(paste("Number of distinct paid media channels:", number_of_distinct_channels))

# Check the csv to understand your data's distribution
write.csv(data_formatted, file = "data_formatted.csv", row.names = FALSE)

# Define the vector of allowed paid_media_channels
allowed_channels <- c(
  'smartphone_yahoo',
  'desktop_websites',
  'smartphone_websites',
...
allowed_channels <- c(
  ...
  'smartphone_apps',
  'desktop_network_3',
  'smartphone_network_2',
  'desktop_network_2',
  'desktop_homepages'
)

# Filter the data frame
filtered_data <- data_formatted[data_formatted$paid_media_channels %in% allowed_channels, ]
# View the filtered data
print(head(filtered_data)) # Or View(filtered_data) to open it in a new tab

# Get the distinct values in the "paid_media_channels" column
distinct_channels_2 <- unique(filtered_data$paid_media_channels)
# Count the number of distinct values
number_of_distinct_channels_2 <- length(distinct_channels_2)
# Print the result
print(paste("Number of distinct paid media channels:", number_of_distinct_channels_2))

# Get the distinct values in the "Context" column
distinct_context <- unique(filtered_data$context)
# Print the result
print(paste(distinct_context))

# Filter brand
selected_brand <- "Brand" # Change this value as needed

brand_bucket <- filtered_data %>%
  filter(brand == selected_brand) %>%
  select(-brand, -context)  # Drop columns after filtering

# Pivot paid_media_channels
brand_data <- brand_bucket %>%
  select(week_start_date, paid_media_channels, impressions, spend_usd, everything()) %>%
  pivot_wider(
    names_from = paid_media_channels,
    values_from = c(impressions, spend_usd),
    values_fn = sum,
    values_fill = list(
       impressions = 0,                # Ensure missing values default to 0
       # value_from_conversions = 0,  
       conversions = 0,
       spend_usd = 0  
     )
  )

write.csv(brand_bucket, file = "brand_bucket.csv", row.names = FALSE)

# Check the csv to understand your data's distribution
write.csv(brand_data, file = "brand_data", row.names = FALSE)

# Filter Context Variables
context_bucket <- data_formatted %>%
  filter(context!= "Brand") %>%
  select(-paid_media_channels, -brand)

# Pivot context variables
context_data <- context_bucket %>%
  select(week_start_date, everything()) %>%
  pivot_wider(
    names_from = context,
    values_from = c(impressions, conversions, spend_usd),
    values_fn = sum,
    values_fill = list(
      impressions = 0,                # Ensure missing values default to 0
      # value_from_conversions = 0,
...
write.csv(context_data, file = "context_data", row.names = FALSE)

brand_data_agg <- brand_data %>%
  group_by(week_start_date) %>%
  summarise(across(where(is.numeric), \(x) sum(x, na.rm = TRUE)), .groups = "drop")  # Corrected syntax

# Join the aggregated brand_data with context_data
final_data <- brand_data_agg %>%
  left_join(context_data, by = "week_start_date") %>%
  arrange(week_start_date)  # Sort from oldest to newest

# Remove spaces from column name
colnames(final_data) <- gsub(" ", "_", colnames(final_data))

# Check for highly correlated vairables
vif_values <- vif(lm(conversions ~ ., data = final_data))
print(vif_values)

# Replace all NA values with the median of each column
# final_data <- final_data %>%
#   mutate(across(everything(), ~replace_na(.x, median(.x, na.rm = TRUE))))

# Save the final data set

write.csv(final_data, file = "final_data", row.names = FALSE)
...
InputCollect <- robyn_inputs(
  ...
  date_var = "week_start_date", # date format must be "2020-01-01"
  ...
  context_vars = c(
  'conversions_personal_finance',
  'conversions_personal_investing'
  ...
  paid_media_spends = c(
    'spend_usd_desktop_homepages',
    'spend_usd_desktop_websites',
    'spend_usd_smartphone_websites',
    'spend_usd_smartphone_apps',
    'spend_usd_desktop_network_3',
    'spend_usd_smartphone_network_2',
    'spend_usd_desktop_network_2',
    'spend_usd_desktop_network_1'
  ), # mandatory input
  paid_media_vars = c(
    'impressions_desktop_homepages',
    'impressions_desktop_websites',
    'impressions_smartphone_websites',
    'impressions_smartphone_apps',
    'impressions_desktop_network_3',
    'impressions_smartphone_network_2',
    'impressions_desktop_network_2',
    'impressions_desktop_network_1'
  ), # mandatory.
...
# High amounts of zero activity
# colSums(is.na(data_clean[, paid_media_spends]))
# colSums(data_clean[, paid_media_spends] == 0, na.rm = TRUE)
# Low-signal variables or not useful to the analysis
# low_signal <- names(data_clean[, paid_media_spends])[colSums(data_clean[, paid_media_spends] == 0) >= 90]
# print(low_signal)
...
OutputModels <- robyn_run(
  ...
  cores = 1, # NULL defaults to (max available - 1)
...
dir.exists(robyn_directory)  # Should return TRUE
dir.create(robyn_directory, recursive = TRUE)
file.create(file.path(robyn_directory, "test.txt"))  # Should return TRUE
unlink(file.path(robyn_directory, "test.txt"))  # Deletes test file
options(mc.cores = 1)  # Force R to use only 1 core globally
Sys.setenv(R_FUTURE_PLAN = "sequential")  # Forces sequential processing
Sys.setenv(R_PARALLEL_NUM_THREADS = "1")  # Limits threads to 1
...
OutputCollect <- robyn_outputs(
  ...
  min_candidates = 150, # top pareto models for clustering. Default to 100
...
print(OutputCollect)
select_model <- "2_1987_1" # Pick one of the models from OutputCollect to proceed

json_file <- "~/Robyn/Robyn_202502251816_init/RobynModel-2_1987_1.json"
...
AllocatorCollect1 <- robyn_allocator(
  ...
  total_budget = 380000, # When NULL, default is total spend in date_range
  channel_constr_low = 0.5,
  channel_constr_up = 1.5, # c(1.2, 1.5, 1.5, 1.5, 1.5),
...
AllocatorCollect2 <- robyn_allocator(
  ...
  total_budget = 380000, # Total budget for date_range period simulation
  channel_constr_low = 0.5, #c(0.8, 0.7, 0.7, 0.7, 0.7),
  channel_constr_up = 1.5, #c(1.2, 1.5, 1.5, 1.5, 1.5),
